...
import { getCategoryActivitiesPath, getProfileIdFromSearch } from "@shared/profileRoute";
...
function DrawingPad({ onDraw, requiredStrokes, currentStrokes, drawingGoal }: { onDraw: () => void; requiredStrokes: number; currentStrokes: number; drawingGoal: string }) {
  ...
  return <div><canvas ref={canvasRef} aria-label="A drawing canvas for this activity" className="drawing-grid h-64 w-full touch-none rounded-[1.65rem] border-2 border-dashed border-[#e9c6d7] bg-[#fffdf8] sm:h-80" onPointerDown={begin} onPointerMove={move} onPointerUp={() => { drawing.current = false; }} onPointerLeave={() => { drawing.current = false; }} /><p className="mt-3 text-center text-sm font-bold text-[#907990]">{drawingGoal} · Add {requiredStrokes} {requiredStrokes === 1 ? "creative line" : "creative lines"} to finish · {Math.min(currentStrokes, requiredStrokes)}/{requiredStrokes}</p></div>;
}
