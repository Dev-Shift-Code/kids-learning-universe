...
import { CATEGORY_IDS, CURRICULUM, getAgeAppropriateActivityCopy, getCategory, type CategoryId, type CurriculumActivity } from "@shared/curriculumConfig";
...
function DrawingPad({ onDraw, requiredStrokes, currentStrokes }: { onDraw: () => void; requiredStrokes: number; currentStrokes: number }) {
  ...
  return <div><canvas ref={canvasRef} aria-label="A drawing canvas for this activity" className="drawing-grid h-64 w-full touch-none rounded-[1.65rem] border-2 border-dashed border-[#e9c6d7] bg-[#fffdf8] sm:h-80" onPointerDown={begin} onPointerMove={move} onPointerUp={() => { drawing.current = false; }} onPointerLeave={() => { drawing.current = false; }} /><p className="mt-3 text-center text-sm font-bold text-[#907990]">Make {requiredStrokes} {requiredStrokes === 1 ? "creative line" : "creative lines"} to complete this art activity · {Math.min(currentStrokes, requiredStrokes)}/{requiredStrokes}</p></div>;
}

function activityPrompt(activity: CurriculumActivity, categoryTitle: string, ageGroup: AgeGroup) {
  const title = activity.title;
  if (activity.interaction === "drawing") return { instruction: getAgeAppropriateActivityCopy(ageGroup, title), helper: `${categoryTitle} is your canvas today. Make a brave, colorful idea and complete the drawing goal.`, target: "drawn", options: [] as string[] };
  if (activity.interaction === "drag-and-drop") return { instruction: `Drag the best card into the glowing spot for ${title}.`, helper: getAgeAppropriateActivityCopy(ageGroup, title), target: "Best match", options: ["Best match", "Try this", "Different card", "Not this one"] };
  return { instruction: `Choose the best answer for ${title}.`, helper: getAgeAppropriateActivityCopy(ageGroup, title), target: "Best answer", options: ["Best answer", "Another answer", "Maybe this", "Keep looking"] };
}
...
export default function Activity() {
  ...
  const prompt = activityPrompt(selectedActivity, category.title, ageGroup);
  ...
  const chooseOption = (option: string) => {
    setChosen(option);
    if (option !== prompt.target) setIncorrectAttempts((count) => count + 1);
  };
  ...
  const finishActivity = async () => {
    ...
    const result = await complete.mutateAsync({ childProfileId: profile.id, category: categoryId, levelNumber: level, interactionType: selectedActivity.interaction, stars, durationSeconds });
